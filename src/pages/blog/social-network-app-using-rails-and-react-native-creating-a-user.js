...
`}</pre>
          ...
          <pre>{`2.6.5 :001 > User
 => User (call 'User.connection' to establish a connection)`}</pre>
          <Paragraph>
          ...
          <pre>{`2.6.5 :029 > user = User.new(email: 'a@abc.com')
...
2.6.5 :030 > user.valid?
 ...
 => #<ActiveModel::Errors:0x00007fa0b21c9418 @base=#<User id: nil, email: "a@abc.com", created_at: nil, updated_at: nil>, @messages={:password=>["can't be blank"]}, @details={:password=>[{:error=>:blank}]}>`}</pre>
          ...
          <pre>{`2.6.5 :038 > user.errors.messages
 => {:password=>["can't be blank"]}`}</pre>
          <Paragraph>
            The error is pretty self explanatory, to prevent it, we would have
            needed to ran:
          </Paragraph>
          <pre>user = User.new(email: 'a@abc.com', password: '123123')</pre>
          <Paragraph>
          ...
          <pre>{`2.6.5 :050 > user.update(password: '123123', password_confirmation: '123123')
   (0.4ms)  BEGIN
